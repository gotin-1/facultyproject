...
export default function EvenementRecent() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/facultyproject/")
      ...
      .then((data) => {
        // Ensure data is an array and has the necessary structure
        if (Array.isArray(data)) {
          const formattedData = data.map((event) => ({
            ...event,
            date: event.date || new Date().toISOString().split("T")[0], // Fallback for date
          }));
          setEvenements((prev) => [...prev, ...formattedData]);
        }
      })
